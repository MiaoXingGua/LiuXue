...
- (void)setHighlight:(BOOL)highlight
{
	...
	if(highlight)
	{
        ...
        _highlightView.center = CGPointMake(_stackIimageView.center.x, _stackIimageView.center.y);
...
- (id)initWithFrame:(CGRect)frame withTitle:(NSString *)title withImage:(UIImage *)image alignment:(UITextAlignment)alignment
{
    ...
    if (self)
	{
		...
		rect = CGRectMake(alignment == UITextAlignmentLeft ? -50 : frame.size.height + 5, 0, frame.size.width - frame.size.height - 5, frame.size.height - 4);
        
		_stackTitleLabel = [[UILabel alloc] initWithFrame:rect];
		[_stackTitleLabel setTextAlignment:NSTextAlignmentRight];
