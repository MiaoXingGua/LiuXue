...
- (void)show:(PCStackMenuBlock)block
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
		for(int i = 0; i < [_items count]; i++)
		{
			...
			r.origin.y += i * r.size.height * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionCounterClockWiseUp ? -1.4 : 10);
            
			stackItem.frame = r;
			CGPoint point = [self centerPointForRotation:stackItem];
			stackItem.transform = CGAffineTransformMakeRotationAt(i * (_menuDirection == PCStackMenuDirectionClockWiseUp || _menuDirection == PCStackMenuDirectionClockWiseDown ? 1 : -0.7) * M_PI / 180, point);
			stackItem.alpha = 1.0;
...
- (void)dismissWithSelect:(PCStackMenuItem *)selectedItem
{
    ...
    CGRect r = ((UIView *)[_items objectAtIndex:selectedIndex]).frame;
    stackItem.transform = CGAffineTransformMakeRotationAt(0 * M_PI / 180, CGPointMake(r.origin.x, r.origin.y));
    stackItem.frame = r;
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        if(selectedItem == stackItem)
        {
            ...
            stackItem.transform = CGAffineTransformMakeScale(1.2, 1.2);
        }
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            stackItem.transform = CGAffineTransformMakeScale(1, 1);
...
- (void)postdismissNotifation
{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"dismissOperationView" object:nil];
}
...
{
    UIWindow *_window = [[[UIApplication sharedApplication] windows] lastObject];
    
    _overlayWindow = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
	_overlayWindow.backgroundColor = [UIColor clearColor];
    [_window addSubview:_overlayWindow];
