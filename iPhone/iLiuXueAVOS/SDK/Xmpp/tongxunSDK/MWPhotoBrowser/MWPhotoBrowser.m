...
- (void)viewDidLoad {
    ...
    [cancelBtn addTarget:self action:@selector(cancelVC) forControlEvents:UIControlEventTouchUpInside];
    [_toolbar addSubview:cancelBtn];
    
    totlaPage = [self numberOfPhotos];
    
    pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-70, 200, 20)];
    pageLabel.backgroundColor = [UIColor clearColor];
    pageLabel.textColor = [UIColor whiteColor];
    pageLabel.font = [UIFont systemFontOfSize:14];
    pageLabel.center = CGPointMake(160, pageLabel.center.y);
    [pageLabel setTextAlignment:NSTextAlignmentCenter];
    pageLabel.text = [NSString stringWithFormat:@"— %d / %d —",selectPage,totlaPage];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    selectPage = scrollView.contentOffset.x/320+1;
    pageLabel.text = [NSString stringWithFormat:@"— %d / %d —",selectPage,totlaPage];
    // Checks
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([MFMailComposeViewController canSendMail]) {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        ...
                                                        otherButtonTitles:NSLocalizedString(@"保存到相册", nil), nil] autorelease];
            } else {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil
