//
//  ReadNotificationViewController.m
//  ILiuXue
//
...
@interface ReadNotificationViewController ()

@end

@implementation ReadNotificationViewController

@synthesize readdatalist = _readdatalist;
...
- (void)dealloc
{
    ...
    [_readdatalist release]; _readdatalist=nil;
...
- (void)viewDidLoad
{
    ...
    self.readdatalist = [NSMutableArray arrayWithCapacity:0];
...
- (void)requestCollectNum
{
    ...
    {
        [self requestNotificationIsRead];
    ...
    [[ALThreadEngine defauleEngine] getMyFaviconThreadNotContainedIn:nil block:^(NSArray *threads, NSError *error) {
        ...
        [bself requestNotificationIsRead];
...
- (void)requestNotificationIsRead
{

    __block typeof(self) bself = self;
    
    __block UITableView *__tableivew = _tableView;
    
    [[ALNotificationCenter defauleCenter] getNotificationsOfThreadNotContainedIn:nil type:ALThreadNotificationTypeOfThreadNewPost isUnread:NO block:^(NSArray *notifications, NSError *error) {
        ...
        if (counts==0 && !error)
        {
            ...
            [bself.readdatalist removeAllObjects];
            
            [bself hideF3HUDSucceed:nil];
            bself.isRequest = NO;
            [__tableivew reloadData];
...
- (void)loadMessages:(NSArray *)array
{
    ...
    [self.readdatalist removeAllObjects];

    [self.readdatalist addObjectsFromArray:ary];
...
- (void)showMoreNotification:(UIButton *)sender
{
    ...
    for (int i=0; i<self.readdatalist.count; i++)
    {
        [array addObject:[[self.readdatalist objectAtIndex:i] objectForKey:@"notification"]];
    }
    ...
    [[ALNotificationCenter defauleCenter] getNotificationsOfThreadNotContainedIn:array type:ALThreadNotificationTypeOfThreadNewPost isUnread:NO block:^(NSArray *notifications, NSError *error) {
    ...
    for (int i=0; i<counts; i++)
    {
        ...
        if(_content)
        {
            ...
            if (_text)
            {
                ...
                if (_dic)
                {
                    [self.readdatalist addObject:_dic];
                }
            }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    temp = [NSDictionary dictionaryWithDictionary:[self.readdatalist objectAtIndex:indexPath.row]];
...
{
    return self.readdatalist.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSDictionary *temp = [NSDictionary dictionaryWithDictionary:[self.readdatalist objectAtIndex:indexPath.row]];
...
{
    NSDictionary *temp = [NSDictionary dictionaryWithDictionary:[self.readdatalist objectAtIndex:sender.tag]];
    ...
    if (_notification)
    {
        ...
        [[ALNotificationCenter defauleCenter] deleteStateOfNotification:_notification block:^(BOOL succeeded, NSError *error) {
            ...
            if (succeeded && !error)
            {
                ...
                [bself.readdatalist removeObjectAtIndex:sender.tag];
...
- (void)playCellVoice:(UIButton *)sender
{
    ...
    self.voiceData = [[self.readdatalist objectAtIndex:sender.tag] objectForKey:@"content"];
