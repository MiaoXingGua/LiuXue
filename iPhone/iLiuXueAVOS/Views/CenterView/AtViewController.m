...
@synthesize atdatalist = _atdatalist;
...
- (void)dealloc
{
    ...
    [_atdatalist release]; _atdatalist=nil;
...
- (void)viewDidLoad
{
    ...
    self.atdatalist = [NSMutableArray arrayWithCapacity:0];
...
- (void)requestNotificationAt
{
    ...
    [[ALNotificationCenter defauleCenter] getNotificationsOfThreadNotContainedIn:nil type:ALThreadNotificationTypeOfAt isUnread:YES block:^(NSArray *notifications, NSError *error) {
        ...
        if (counts==0 && !error)
        {
            ...
            [bself.atdatalist removeAllObjects];
...
- (void)loadMessages:(NSArray *)array
{
    ...
    [self.atdatalist removeAllObjects];

    [self.atdatalist addObjectsFromArray:ary];
...
- (void)showMoreNotification:(UIButton *)sender
{
    ...
    for (int i=0; i<self.atdatalist.count; i++)
    {
        [array addObject:[[self.atdatalist objectAtIndex:i] objectForKey:@"notification"]];
    }
    
    __block UIButton *__loadbtn = loadCommentsBtn;
    __block UIActivityIndicatorView *__activity = _activityView;
    
    [[ALNotificationCenter defauleCenter] getNotificationsOfThreadNotContainedIn:array type:ALThreadNotificationTypeOfAt isUnread:YES block:^(NSArray *notifications, NSError *error) {
    ...
    for (int i=0; i<counts; i++)
    {
        ...
        if(_content)
        {
            ...
            if (_text)
            {
                ...
                if (_dic)
                {
                    [self.atdatalist addObject:_dic];
                }
            }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    temp = [NSDictionary dictionaryWithDictionary:[self.atdatalist objectAtIndex:indexPath.row]];
...
{
    return self.atdatalist.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSDictionary *temp = [NSDictionary dictionaryWithDictionary:[self.atdatalist objectAtIndex:indexPath.row]];
...
{
    NSDictionary *temp = [NSDictionary dictionaryWithDictionary:[self.atdatalist objectAtIndex:sender.tag]];
    ...
    if (_notification)
    {
        ...
        [[ALNotificationCenter defauleCenter] deleteStateOfNotification:_notification block:^(BOOL succeeded, NSError *error) {
            ...
            if (succeeded && !error)
            {
                ...
                [bself.atdatalist removeObjectAtIndex:sender.tag];
...
- (void)playCellVoice:(UIButton *)sender
{
    ...
    self.voiceData = [[self.atdatalist objectAtIndex:sender.tag] objectForKey:@"content"];
